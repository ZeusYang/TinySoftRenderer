...
{
public:
    Vector4D posTrans;  // Position in world space.
    Vector4D posH;      // Projected position.
    Vector2D texcoord;  // Texture coordinates.
    Vector3D normal;	// Vertex normal
    Vector4D color;	    // Vertex color.
    double oneDivZ;     // One divided by z.
...
public:
    ...
    std::vector<unsigned int> m_indices;// Mesh indices.
